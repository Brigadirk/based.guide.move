...
export function PersonalInfoForm({ data, onUpdate }: PersonalInfoFormProps) {
  ...
  const handleChange = <K extends keyof PersonalInformation>(
    field: K,
    value: PersonalInformation[K]
  ) => {
    setInfo(prev => ({ ...prev, [field]: value }))
    setTouched(prev => ({ ...prev, [field]: true }))
    onUpdate({ personalInformation: { ...info, [field]: value } })
  }
  ...
  const removeNationality = (index: number) => {
    const updated = info.nationalities.filter((_, i) => i !== index)
    handleChange("nationalities", updated)
  }
            ...
            <Select
              ...
              onValueChange={(value: ResidencyStatus) => {
                handleChange("currentResidency", {
                  ...info.currentResidency,
                  status: value
                } as PersonalInformation["currentResidency"])
